...
4. **Retraining cost** — Walking Claude through the two-app testing setup (test_app + compose-dev/admin) every time.
...
| `integration-testing.md` | `lib/codebase_index/extractors/**/*.rb`, `spec/**/*_spec.rb` | Host app validation workflow (test_app + admin) |
...
| Agent | Tools | Purpose |
|---|---|---|
| `host-app-validator` | Bash, Read, Glob, Grep | Validates extraction in test_app (host) or compose-dev/admin (Docker). Takes environment as parameter. |
...
Files with direct references to local projects (compose-dev, bc_admin) are gitignored:
...
4. **Validate** — Dispatch host-app-validator agent to test_app or admin with a specific plan.
...
- **Single validator agent, environment as parameter.** test_app and admin validation share the same workflow shape — only the commands differ.
...
- **Privacy via .gitignore.** Files referencing local project paths (compose-dev, container names) are not committed.
